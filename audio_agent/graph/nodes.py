...
def create_question_clarification_node(planner: BasePlanner):
    ...
    def question_clarification_node(state: AgentState) -> dict:
        ...
        log_node_start("question_clarification_node", {
            "question": state.get("question", "")[:50],
        })
        ...
        log_node_end("question_clarification_node", {
...
def create_frontend_evidence_node(frontend: BaseFrontend):
    ...
    def frontend_evidence_node(state: AgentState) -> dict:
        ...
        if use_dual:
            ...
            direct_prompt = question_oriented_prompt or ""
            if clarification:
                direct_prompt += f"\n\n[Analysis] Question type: {clarification.question_type}"
                if clarification.suggested_focus:
                    direct_prompt += f"\nFocus areas: {', '.join(clarification.suggested_focus)}"
                if clarification.requires_cot:
                    direct_prompt += "\n\nInclude your step-by-step reasoning process."
                else:
                    direct_prompt += "\n\nAnswer directly without explaining your reasoning."

            if hasattr(frontend, "run_direct_answer"):
                output_direct = frontend.run_direct_answer(question, audio_paths, direct_prompt)
            else:
                # Fallback for frontends that don't implement run_direct_answer yet
                output_direct = frontend.run(question, audio_paths, direct_prompt)
        ...
        log_node_end("frontend_evidence_node", {
            "caption_length": len(output_caption.question_guided_caption),
            "observer_present": output_direct is not None,
            "direct_answer_length": len(output_direct.question_guided_caption) if output_direct else 0,
        })
