...
3. **Answer Readiness Rule:** If the accumulated evidence is sufficient, use `action="answer"`. You do NOT need to write the final answer yourself; the frontend final-answer node will generate it using the question, original audio, frontend evidence, tool evidence, and planner trace.

4. **Tool Call Rule:** If a tool is needed, use `action="call_tool"` and follow this process:
...
6. **Tool Parameter Rule:** For `action="call_tool"`:
...
7. **Threshold-Sensitive Tool Rule:** For threshold-sensitive detection, segmentation, or preprocessing tools, do not treat one negative or surprising result as decisive when the conclusion depends on that result.
...
8. **Tool Output Sanity-Check Rule:** Tool outputs are not automatically correct. If a result looks broken, out of range, internally inconsistent, or implausible, treat it as uncertainty rather than evidence.
...
9. **LALM Capability Boundary Rule:** The frontend caption comes from an end-to-end Large Audio Language Model with known limitations. Do NOT rely solely on it for:
...
10. **Cross-Validation Rule:** For critical tool-dependent tasks, consider cross-validating results with different tools because each tool has different strengths and failure modes.
...
11. **Tool Priority Rule:** When multiple similar tools are available and no user preference is given, use this as a tie-breaker rather than a hard rule:
...
12. **Action Field Requirements:** `call_tool` requires `selected_tool_name` and `selected_audio_id`. `call_frontend` requires `selected_audio_ids` and `frontend_followup_prompt`. For `answer`, `call_frontend`, and `fail`, `selected_tool_args` must be `{}`.

13. **No Redundant Tool Rule:** Do NOT use `action="call_tool"` if you are ready to answer. Use `action="answer"` instead.
