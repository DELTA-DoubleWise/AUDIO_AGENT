#!/usr/bin/env python3
"""
Demo script with Xiaomi MiMo API-based frontend and planner.

This script demonstrates the complete workflow using:
- MiMo frontend (mimo-v2.5 via MiMo's OpenAI-compatible API)
- MiMo planner (mimo-v2.5-pro via MiMo's OpenAI-compatible API)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Run the audio agent demo with MiMo API-based frontend, planner, and auto-registered MCP tools.")
    parser.add_argument(
        "--audio",
        required=True,
        nargs="+",
    ...
    parser.add_argument(
        "--frontend-model",
        default="mimo-v2.5",
        help="API model name for frontend (default: mimo-v2.5).",
    )
    parser.add_argument(
        "--planner-model",
        default="mimo-v2.5-pro",
        help="API model name for planner (default: mimo-v2.5-pro).",
    )
    parser.add_argument(
        "--api-key",
        default=None,
        help="API key. If not provided, reads from MIMO_API_KEY env var.",
    )
    parser.add_argument(
        "--base-url",
        default="https://token-plan-cn.xiaomimimo.com/v1",
        help="API base URL (default: https://token-plan-cn.xiaomimimo.com/v1).",
    )
    parser.add_argument(
        "--enable-thinking",
        action="store_true",
...
async def amain() -> int:
    ...
    print_separator("Audio Agent Framework Demo (MiMo API Mode)")
    print("\nThis demo runs the agent with:")
    print(f"- {args.frontend_model} frontend (MiMo API-based)")
    print(f"- {args.planner_model} planner (MiMo API-based)")
    ...
    if not api_key:
        print("Error: API key required. Provide via --api-key or set MIMO_API_KEY environment variable.")
        return 1
    ...
    if len(audio_paths) == 1:
        ...
        for i, path in enumerate(audio_paths, 1):
            print(f"  [{i}] {path}")
    print(f"Frontend model: {args.frontend_model} (MiMo API)")
    print(f"Planner model: {args.planner_model} (MiMo API)")
