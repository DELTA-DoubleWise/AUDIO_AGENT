...
#   sbatch -p gpu.normal --mem=32GB --gres=gpu:1 --time=02:00:00 \
...
# Try to discover and source user's conda hook.
if ! command -v conda >/dev/null 2>&1; then
    for candidate in \
        "${CONDA_SH:-}" \
        "/itet-stor/yuchwang/net_scratch/conda/etc/profile.d/conda.sh" \
...
MAIN_ENV="${AUDIO_AGENT_MAIN_ENV:-/itet-stor/yuchwang/net_scratch/conda_envs/audio_agent}"
if [ ! -x "$MAIN_ENV/bin/python" ]; then
    log "ERROR: main env python not found at $MAIN_ENV/bin/python"
    log "Set AUDIO_AGENT_MAIN_ENV to your audio_agent conda env."
    exit 1
fi
